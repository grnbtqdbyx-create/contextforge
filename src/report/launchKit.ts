...
export function createLaunchKit(options: LaunchKitOptions): string {
  ...
  const lines = [
    ...
    '## One-Liner',
    '',
    `${projectName} is a local-first context readiness gate for Codex, Claude Code, and GitHub Copilot repositories.`,
    '',
    '## Proof Commands',
    '',
    '```bash',
    ...
    '',
    `${projectName} turns that layer into a deterministic readiness check for Codex, Claude Code, and GitHub Copilot: context health, cache stability, trace efficiency, prompt-injection safety, agentic workflow injection checks, review kits, public proof files, community health files, CI artifacts, and task-specific context packs.`,
    '',
    'Try the proof path:',
    '',
    '```bash',
    ...
    '- `contextforge launch-snapshot --output docs/launch-snapshot.md` gives README visitors the current why-now, adjacent-category, and proof-first story.',
