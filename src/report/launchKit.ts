...
export function createLaunchKit(options: LaunchKitOptions): string {
  ...
  const lines = [
    ...
    '',
    `${projectName} turns that layer into a deterministic readiness check for Codex and Claude Code: context health, cache stability, prompt-injection safety, review kits, public proof files, community health files, CI artifacts, and task-specific context packs.`,
    '',
    'Try the proof path:',
    '',
    '```bash',
