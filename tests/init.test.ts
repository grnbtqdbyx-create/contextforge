...
describe('GitHub Action init scaffold', () => {
  it('writes a reusable ContextForge workflow with artifacts and SARIF upload', async () => {
    ...
    expect(workflow).toContain('surface-inventory: contextforge-agent-surface-inventory.md');
    expect(workflow).toContain('surface-diff: contextforge-agent-surface-diff.md');
    ...
    expect(workflow).toContain('workflow-audit: contextforge-workflow-audit.md');
    expect(workflow).toContain('workflow-sarif: contextforge-workflow.sarif');
    expect(workflow).toContain('actions-audit: contextforge-actions-audit.md');
    expect(workflow).toContain('actions-sarif: contextforge-actions.sarif');
    expect(workflow).toContain('trace-audit: contextforge-trace-audit.md');
  ...
  it('uses the latest released action ref by default', async () => {
    ...
    expect(await readFile(result.path, 'utf8')).toContain('uses: grnbtqdbyx-create/contextforge@v0.70.0');
  });
