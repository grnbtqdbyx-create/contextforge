...
instructions, Copilot prompt files, custom agents, and project skills for
prompt/context poisoning, and creates task-specific context packs for Codex,
...
For GitHub Copilot customization coverage, see
...
The scorecard is the short README-ready snapshot. The proof pack combines
doctor checks, audit scores, evidence commands, and a
...
For a first-time maintainer or OSS reviewer deciding whether the project is
worth trying:
...
That file gives the 30-second proof path, adjacent-tool positioning, try-it
...
That file flags hardcoded MCP secrets, remote shell installers, and unpinned
package launches before Codex, Claude, or another coding agent loads the repo's
tooling.
...
That file flags risky project `defaultMode`, broad Bash allow rules, remote
shell hooks, wildcard HTTP hook destinations, and missing sensitive-file deny
rules before Claude Code users trust shared repo settings.

For repeated tool calls, bulky tool output, and low cache reuse in Codex/Claude
traces:
...
- **Audit repo instructions:** keep root `README.md`, nested `AGENTS.md`, `CLAUDE.md`, `.github/copilot-instructions.md`, `.github/instructions/**/*.instructions.md`, `.github/prompts/**/*.prompt.md`, `.github/agents/**/*.md`, `.github/agents/**/*.agent.md`, project `SKILL.md` files, `.cursorrules`, and `.clinerules` useful instead of bloated or unsafe.
...
`CLAUDE.md`, and `.github/copilot-instructions.md`. The PR comment workflow
remains a separate file because posting comments requires `pull-requests: write`.
...
The evaluator adoption brief lives in [docs/adoption.md](docs/adoption.md).
...
Scorecard generation is documented in [docs/scorecard.md](docs/scorecard.md).
Review-kit generation is documented in [docs/review-kit.md](docs/review-kit.md).
MCP exposure audits are documented in [docs/mcp-audit.md](docs/mcp-audit.md).
Claude Code settings audits are documented in [docs/claude-audit.md](docs/claude-audit.md).
GitHub Copilot customization coverage is documented in [docs/copilot-instructions.md](docs/copilot-instructions.md).
...
ContextForge learns from popular tools like Repomix, ccusage, GitHub Copilot
custom instructions, AGENTS.md, context-mode, Claude Context, and LLMLingua, but focuses on a narrower gap:
...
ContextForge v0.55.0 is a public MVP CLI with:
...
- context security audit with nested monorepo, GitHub Copilot customization discovery, project skill scanning, and root README injection checks
